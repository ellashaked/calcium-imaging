...
def detect_influx_start_index(trace: pd.Series) -> int:
    end_idx = detect_influx_end_index(trace)
    start_idx = end_idx - INFLUX_START_INDEX_MIN_OFFSET_FROM_END
    while start_idx > end_idx - INFLUX_START_INDEX_MAX_OFFSET_FROM_END:
        if trace.iloc[start_idx] <= 1.0:  # at or below baseline fluorescence level
            return start_idx
        start_idx -= 1
    return start_idx  # end_idx - INFLUX_START_INDEX_MAX_OFFSET_FROM_PEAK


def detect_influx_end_index(trace: pd.Series) -> int:
    return detect_peak_index(trace) - INFLUX_END_INDEX_OFFSET_FROM_PEAK
